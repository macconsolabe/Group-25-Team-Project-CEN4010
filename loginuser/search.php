...
<head>
    <title>Search</title>
    <link rel="icon" type="image/x-icon" href="assets/thunder.png" />
...
<body>
    <div class="container">
        ...
        if ($location == 'emails') {
            ...
            include 'includes/userquery.php';  /// query search function
        } else if ($location == 'names') {   ////search by name
            $name = explode(' ', $key, 2); // Break String into Array.            EVERY SEARCH IS MADE BY CHARACTER
            if (empty($name[1])) {
        ...
        } else if ($location == 'posts') {   ///search by post
            ...
            if (mysqli_num_rows($query) == 0) {
                echo '<div class="post">';
                echo 'There is no results given the keyword, try to widen your search query.';   ///ERROR MSG
                echo '</div>';
                echo '<br>';
            }
            while ($row = mysqli_fetch_assoc($query)) {
                include 'includes/post.php';  ///include post function to search
                echo '<br>';
            }
        }
        ?>
